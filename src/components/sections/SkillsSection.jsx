...
import { SKILLS_CATEGORY, STATS, TECKSTACK } from '../../utils/data';
    ...
    return (
        ...
        >
            ...
            <motion.div style={{ y }} className='absolute inset-0 overflow-hidden'>
                <div className={`absolute top-40 right-1/4 w-72 h-72 rounded-full blur-3xl opacity-5 ${isDarkMode ? "bg-blue-500" : "bg-blue-400"}`}></div>
                <div className={`absolute bottom-40 left-1/4 w-64 h-64 rounded-full blur-3xl opacity-5 ${isDarkMode ? "bg-purple-500" : "bg-purple-500"}`}></div>
            </motion.div>
            <div className='max-w-6xl mx-auto relative z-10'>
                ...
                <motion.div initial="hidden"
                    ...
                    <motion.h2 variants={itemVariants}
                        className='text-3xl md:text-5xl font-light mb-6'>Skills &
                ...
                >
                    ...
                    <motion.div variants={itemVariants} className='flex flex-wrap justify-center gap-3'>
                        {TECKSTACK.map((tech, index) => (
                            <motion.span key={tech} whileHover={{ y: -2, scale: 1.05 }}
                                className={`px-4 py-2 text-sm rounded-full border transition-all duration-300 ${isDarkMode ? "bg-gray-900 border-gray-700 text-gray-300 hover:border-gray-600" : "bg-white border-gray-300 text-gray-700 hover:border-gray-400"}`}>{tech}</motion.span>
                        ))}
                    </motion.div>
                </motion.div>
